...
#It was pretty good. Just about 8cm closer to the fridge
...
class auto_park:
	...
	def approach1(self, dist, data):
		if not self.donewith1:
			...
			if ((dist-0.02)>z_dist_1) or (z_dist_1 > (dist+0.02)):
				velocity = Twist()

				#0.05 in order to prevent arm from hitting table
				if z_dist_1 < (dist-0.02):
					velocity.linear.x = -1.0
				elif z_dist_1 > (dist+0.02):
					velocity.linear.x = 1.0
	...
	def approach5(self, dist, data):
		if not self.donewith5:
			...
			if ((dist-0.02)>z_dist_5) or (z_dist_5 > (dist+0.02) or (x_dist_5 < -0.02) or (x_dist_5 > 0.02)):
				velocity = Twist()
	...
	def update_markers(self, data):
		...
		for i in range(0, len(data.markers)):
			if data.markers[i].id == 1:
				if not self.donewith1:
					self.approach1(1.68, data.markers[i])
